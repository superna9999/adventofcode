...
struct _grid {
	...
	unsigned int already_won;
} grids[MAX_GRIDS];

int last_win_grid = -1;
unsigned int last_win_sum;
...
int mark_grid(int grid, unsigned int number)
{
	...
	for (col = 0; col < GRID_W; ++col) {
		...
		if (ln == GRID_W) {
			...
			last_win_grid = grid;
			last_win_sum = grid_sum(grid) * number;
			return 0;
		}
	}
...
unsigned int run_draws(char *draws, unsigned int grid_count)
{
	char *data;
	char *pos, *tok;
	unsigned grid;
	...
	do {
		tok = strsep(&pos, ",");
		for (grid = 0 ; grid < grid_count ; ++grid) {
			if (grids[grid].already_won)
				continue;
			ret = mark_grid(grid, strtoul(tok, NULL, 10));
			if (ret == 0)
				 grids[grid].already_won = 1;
		}
	} while (pos);
...
int main(int argc, char *argv[])
{
	...
	printf("Last Grid win: %d Score: %d\n", last_win_grid, last_win_sum);
